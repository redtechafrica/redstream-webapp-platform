...
export default function RatingPage() {
  ...
  const { slug } = params
  const ratingSlug = Array.isArray(slug) ? slug[0] : slug
  ...
  useEffect(() => {
    ...
    const videos = Object.values(mockVideos).filter((video) => video.rating.toLowerCase() === ratingSlug?.toLowerCase())
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 pt-24 pb-12">
        <h1 className="text-3xl font-bold mb-8">{ratingSlug} Rated Movies & Shows</h1>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <h2 className="text-xl font-medium mb-2">No {ratingSlug} rated content found</h2>
            <p className="text-gray-400">Try searching for a different rating or check back later for new content.</p>
          </div>
        )}
      </div>
